...
impl<P: ThreadPool> KvsEngine for SledKvsEngine<P> {
    fn set(&self, key: String, value: String) -> Box<dyn Future<Output = Result<()>> + Send> {
    ...
    fn get(&self, key: String) -> Box<dyn Future<Output = Result<Option<String>>> + Send> {
        let db = self.db.clone();
        let (tx, rx) = oneshot::channel();
        self.pool.spawn(move || {
            let res = (move || {
    ...
    fn remove(&self, key: String) -> Box<dyn Future<Output = Result<()>> + Send> {
        let db = self.db.clone();
        let (tx, rx) = oneshot::channel();
        self.pool.spawn(move || {
            let res = (|| {
